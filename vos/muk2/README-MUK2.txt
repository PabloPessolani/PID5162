MINIX AS A UNIKERNEL (MUK) CON CORUTINAS Y THREADS 
==================================================

El objetivo es construir un Unikernel conformado por todos los servidores y tareas de  MINIX(MoL)
A diferencia de la primer version de MUK en que el Unikernel en donde cada server/tarea se 
ejecuta en un thread Linux, perteneciente al proceso MUK, en MUK2 se utilizan corutinas 
provistas por la libreria libtask.

Tanto en MUK como en MUK2 se compilan todos los fuentes utilizando y creando librerias estáticas.

Cuando MUK2 inicia, arranca cada una de las tareas/servers como corutinas y luego queda esperando
por la finalizacion de todas.
Al igual que en MUK se puede seleccionar que tareas y servidores arranca en funcion de las constantes 
definidas en glo.h
...
Hay algunas corutinas especiales como muk_tout_task que se encarga de gestionar los timers requeridos
por las primitivas MUK de IPC.

A diferencia de los threads, las corutinas no pueden utilizar servicios de Linux (ni del DVK) 
que sean bloqueantes, porque de esa forma bloquearian al proceso completo.
Solo se pueden utilizar el tasksleep()/taskwakeup().

Para la comunicacion entre las corutinas que conforman MoL se utiliza M3-IPC pero adaptado a libtask
con nombres tales como  muk_send_T(), muk_receive_T(), muk_notify_X(), muk_sendrec_T(),
muk_wait4bind_X.
En tanto que cuando una corutina MUK2 quiere comunicarse con un endpoint externo debe utilizar
M3-IPC adaptado que utiliza threads para no bloquear a las corutinas de MUK2.
Cada corutina que hace bind() dentro de MUK2 tiene asociado un thread que hace bind() en el DVK 
donde ejecuta MUK2. 
La corutina que necesita comunicarse con un endpoint externo a MUK2, utiliza M3-IPC adaptado 
DVK_send_T(), DVK_receive_T(), DVK_notify(), DVK_vcopy().
Cada una de estas funciones construye un requerimiento (estructura dvk_request_t) asociada al descriptor
de la corutina (Task) y despierta a su thread asociado utilizando MUTEXES entre la corutina y el thread
para sincronizacion. Luego, la tarea hace tasksleep().
Una vez despierto el thread asociado, en funcion del requerimiento, realiza la llamada al DVK solicitada,
pudiendo bloquearse o no en esta.

Finalizada la ejecucion de la dvk_call(), almacena el codigo de retorno de error en el descriptor de la 
Task correspondiente, setea el bit correspondiente a la Task en el bitmap dvk_bitmap para indicar que 
el thread de la tarea ya ha concluido con el dvk_call() y que hay que despertar a la tarea. Luego, el 
thread queda a la espera en un MUTEX hasta la proxima peticion.

El taskscheduler de MUK2, cada vez que cambia de corutina (task switching), verifica el bitmap y despierta a las 
corutinas correspondientes.

Los archivos de configuracion MUK y MUK2 son compatibles.


EJECUCION
=========

Se utiliza muk.sh (ver abajo el script) para:
	- arrancar el DVS
	- arrancar el DC0 
