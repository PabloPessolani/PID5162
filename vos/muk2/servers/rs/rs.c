...
#define SELFTASK (-1) 
		...
		switch(deamon_type){
			case LCL_BIND:
				rcode = muk_lclbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep); 
				break;
			case REPLICA_BIND:
				rcode = muk_replbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep); 
				break;
			case BKUP_BIND:
				rcode = muk_bkupbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep,deamon_nodeid);
				break;
			default:
				SVRDEBUG("NEVER HERE!!\n");
				rcode = EDVSINVAL;
