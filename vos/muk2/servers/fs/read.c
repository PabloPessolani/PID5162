...
MUKDEBUG("READING get_block(dev=%d, b=%d, n=%d) \n", dev, b, n);  
  ...
  /* Copy a chunk from the block buffer to user space. */
	//muk_vcopy(fs_who_e, path, fs_ep, user_path, len);  	
    //r = muk_vcopy(fs_ep, D, usr, seg, chunk);
    // r = muk_vcopy(fs_ep, (bp->b_data+off), usr, buff, chunk);
    MUK_vcopy( r, fs_ep, (bp->b_data+off), usr, buff, chunk);
	MUKDEBUG("Resultado muk_vcopy SERVIDOR -> CLIENTE %d\n", r);  
	MUKDEBUG("chunk %d\n", chunk);     
     //MUKDEBUG("buff %s\n", buff);
// MUKDEBUG("bp->b_data+off %s\n", bp->b_data+off);
  ...
  //     (phys_bytes) chunk);
     //r = muk_vcopy(usr, seg, fs_ep, D, chunk);         
	// r = muk_vcopy(usr, buff, fs_ep, (bp->b_data+off), chunk);  
    MUK_vcopy( r, usr, buff, fs_ep, (bp->b_data+off), chunk);  
	MUKDEBUG("Resultado muk_vcopy CLIENTE -> SERVIDOR %d\n", r);  
	MUKDEBUG("chunk %d\n", chunk);  
//MUKDEBUG("buff %s\n", buff);
// MUKDEBUG("bp->b_data+off %s\n", bp->b_data+off);
     bp->b_dirt = DIRTY;
