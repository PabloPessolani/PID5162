 ...
 *   fs_link:         perform the LINK system call
 *   fs_unlink:	      perform the UNLINK and RMDIR system calls
 *   fs_rename:	      perform the RENAME system call
 *   fs_truncate:     perform the TRUNCATE system call
 *   fs_ftruncate:    perform the FTRUNCATE system call
...
/* Perform the link(name1, name2) system call. */
  ...
  if (r == OK)
	if ( (rip->i_mode & I_TYPE) == I_DIRECTORY && !super_user) r = EDVSPERM;
  ...
  if ( (ip = last_dir(user_path, string)) == NIL_INODE) r = fs_err_code;
  ...
  if (r == OK)
	if (rip->i_dev != ip->i_dev) r = EDVSXDEV;
 ...
 */
  ...
  r = OK;
  if ( (rip = advance(&rldirp, string)) == NIL_INODE) r = fs_err_code;
  ...
  if (fs_call_nr == MOLUNLINK) {
	...
	if (rip->i_num == ROOT_INODE) r = EDVSBUSY;
...
/* Perform the rename(name1, name2) system call. */
  ...
  if (fetch_name(fs_m_in.name1, fs_m_in.name1_length, M1) != OK) return(fs_err_code);
  if ( (old_dirp = last_dir(user_path, old_name))==NIL_INODE) return(fs_err_code);

  if ( (old_ip = advance(&old_dirp, old_name)) == NIL_INODE) r = fs_err_code;

  /* See if 'name2' (new name) exists.  Get dir and file inodes. */
  if (fetch_name(fs_m_in.name2, fs_m_in.name2_length, M1) != OK) r = fs_err_code;
  if ( (new_dirp = last_dir(user_path, new_name)) == NIL_INODE) r = fs_err_code;
  ...
  if (r == OK) {
	...
	if (strcmp(old_name, ".")==0 || strcmp(old_name, "..")==0 ||
	    strcmp(new_name, ".")==0 || strcmp(new_name, "..")==0) r = EDVSINVAL;
	...
	if (new_ip == NIL_INODE) {
		...
		if (old_ip->i_dev != old_dirp->i_dev) r = EDVSXDEV;
		if (odir && new_dirp->i_nlinks >=
		    (new_dirp->i_sp->s_version == V1 ? CHAR_MAX : SHRT_MAX) &&
		    !same_pdir && r == OK) r = EDVSMLINK;
	} else {
		if (old_ip == new_ip) r = SAME; /* old=new */

		/* has the old file or new file a file system mounted on it? */
		if (old_ip->i_dev != new_ip->i_dev) r = EDVSXDEV;

		ndir = ((new_ip->i_mode & I_TYPE) == I_DIRECTORY); /* dir ? */
		if (odir == TRUE && ndir == FALSE) r = EDVSNOTDIR;
		if (odir == FALSE && ndir == TRUE) r = EDVSISDIR;
	}
  }
...
int fs_truncate()
{
/* truncate_inode() does the actual work of fs_truncate() and fs_ftruncate().
 * fs_truncate() and fs_ftruncate() have to get hold of the inode, either
 ...
 */
	...
	if (fetch_name(fs_m_in.m2_p1, fs_m_in.m2_i1, M1) != OK)
		return fs_err_code;
	...
	if ( (rip->i_mode & I_TYPE) != I_REGULAR)
		...
		r = truncate_inode(rip, fs_m_in.m2_l1); 
	put_inode(rip);
...
int fs_ftruncate()
{
/* As with fs_truncate(), truncate_inode() does the actual work. */
	struct filp *rfilp;
	if ( (rfilp = get_filp(fs_m_in.m2_i1)) == NIL_FILP)
		return fs_err_code;
	if ( (rfilp->filp_ino->i_mode & I_TYPE) != I_REGULAR)
		return EDVSINVAL;
	return truncate_inode(rfilp->filp_ino, fs_m_in.m2_l1);
}
 ...
 */
  ...
  if (waspipe) {
	if(newsize != 0)
		return EDVSINVAL;	/* Only truncate pipes to 0. */
...
//char dir_name[NAME_MAX];		/* name of directory to be removed */
{
  ...
  if (strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)return(EDVSINVAL);
  if (rip->i_num == ROOT_INODE) return(EDVSBUSY); /* can't remove 'root' */
  
  for (rfp = &fs_proc_table[INIT_PROC_NR + 1]; rfp < &fs_proc_table[dc_ptr->dc_nr_procs]; rfp++)
...
/* Unlink 'file_name'; rip must be the inode of 'file_name' or NIL_INODE. */
  ...
  if (rip == NIL_INODE) {
	...
	fs_err_code = search_dir(dirp, file_name, &numb, LOOK_UP);
	if (fs_err_code == OK) rip = get_inode(dirp->i_dev, (int) numb);
	if (fs_err_code != OK || rip == NIL_INODE) return(fs_err_code);
  } else {
