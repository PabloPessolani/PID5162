 ...
 */
	...
	*k = -1;			/* we need a way to tell if file desc found */
	MUKDEBUG("start=%d mode=%X\n", start,bits);

	/* Search the fs_proc_table fp_filp table for a free file descriptor. */
	for (i = start; i < MNX_OPEN_MAX; i++) {
		if (fs_proc->fp_filp[i] == NIL_FILP && !FD_ISSET(i, &fs_proc->fp_filp_inuse)) {
			/* A file descriptor has been located. */
			MUKDEBUG("FD located is=%d\n", i);
			*k = i;
			break;
		}
	}
  ...
  if (*k < 0) return(EDVSMFILE);	/* this is why we initialized k to -1 */
...
/* See if 'fild' refers to a valid file descr.  If so, return its filp ptr. */
  ...
  fs_err_code = EDVSBADF;
  MUKDEBUG("fild is=%d\n",fild);
  if (fild < 0 || fild >= MNX_OPEN_MAX ) return(NIL_FILP);
  return(fs_proc->fp_filp[fild]);	/* may also be NIL_FILP */
}
...
int inval_filp(struct filp *fs_proc)
{
  ...
  for(f = 0; f < dc_ptr->dc_nr_procs; f++) {
    if(fs_proc_table[f].fp_pid == PID_FREE) continue;
    for(fid = 0; fid < MNX_OPEN_MAX; fid++) {
      if(fs_proc_table[f].fp_filp[fid] && fs_proc_table[f].fp_filp[fid] == fs_proc) {
        fs_proc_table[f].fp_filp[fid] = NIL_FILP;
        n++;
      }
    }
  }
