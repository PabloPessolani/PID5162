 ...
 *   fs_select:	       perform the SELECT system call
...
#define MAXSELECTS 25
 ...
 struct selectentry {
	struct fproc_table *requestor;	/* slot is free iff this is NULL */
	int req_endpt;
...
int select_reevaluate(struct filp *fs_proc);
...
void copy_fdsets(struct selectentry *e)
{
	...
	if (e->vir_writefds)
		MUK_vcopy(r, e->vir_writefds,  &e->ready_writefds, sizeof(fd_set));
	if (e->vir_errorfds)
		MUK_vcopy(r, e->vir_errorfds, &e->ready_errorfds, sizeof(fd_set));
#endif // SYS_VCOPY_FUNCTION
...
int fs_select(void)
{
	...
	selecttab[s].req_endpt = fs_who_e;
	selecttab[s].nfds = 0;
	...
	selecttab[s].vir_readfds = (fd_set *) fs_m_in.SEL_READFDS;
	selecttab[s].vir_writefds = (fd_set *) fs_m_in.SEL_WRITEFDS;
	selecttab[s].vir_errorfds = (fd_set *) fs_m_in.SEL_ERRORFDS;
	...
	if (selecttab[s].vir_readfds){
		MUK_vcopy(r, &selecttab[s].readfds, fs_m_in.SEL_READFDS, sizeof(fd_set));
	}
	
	if (selecttab[s].vir_writefds){
		MUK_vcopy(r, &selecttab[s].writefds,fs_m_in.SEL_WRITEFDS,sizeof(fd_set));
	}

	if (selecttab[s].vir_errorfds){
		MUK_vcopy(r, &selecttab[s].errorfds,fs_m_in.SEL_ERRORFDS, sizeof(fd_set));
	}
	
	if (!fs_m_in.SEL_TIMEOUT)
		is_timeout = nonzero_timeout = 0;
	else
		MUK_vcopy(r, &timeout, fs_m_in.SEL_TIMEOUT,sizeof(timeout));
	...
	for(fid = 0; fid < nfds; fid++) {
		...
		if ((selecttab[s].filps[fid]->filp_select_ops & ops) != ops) {
			...
			if (wantops) {
				 ...
				 * on these operations of this fs_proc, they can
				 * be awoken too
				 */
...
void select_cancel_all(struct selectentry *e)
{
	...
	for(fid = 0; fid < e->nfds; fid++) {
		...
		if (fs_proc->filp_selectors < 1) {
#if DEBUG_SELECT
			printf("select: %d selectors?!\n", fs_proc->filp_selectors);
#endif
			continue;
		}
		fs_proc->filp_selectors--;
		e->filps[fid] = NULL;
		select_reevaluate(fs_proc);
	}
...
int select_reevaluate(struct filp *fs_proc)
{
	...
	if (!fs_proc) {
		printf("fs: select: reevalute NULL fs_proc\n");
		return 0;
	}
	...
	for(s = 0; s < MAXSELECTS; s++) {
		...
		for(fid = 0; fid < selecttab[s].nfds; fid++)
			if (fs_proc == selecttab[s].filps[fid]) {
	 ...
	 */
	fs_proc->filp_select_ops = remain_ops;
#if DEBUG_SELECT
	printf("remaining operations on fs_proc are %d\n", fs_proc->filp_select_ops);
#endif
 ...
 *				select_callback			             *
 *===========================================================================*/
int select_callback(struct filp *fs_proc, int ops)
{
	int s, fid, want_ops, type;

	/* We are being notified that file pointer fs_proc is available for
	...
	for(s = 0; s < MAXSELECTS; s++) {
		...
		for(fid = 0; fid < selecttab[s].nfds; fid++) {
			if (!selecttab[s].filps[fid])
				continue;
			if (selecttab[s].filps[fid] == fs_proc) {
				int this_want_ops;
	...
	for(s = 0; s < MAXSELECTS; s++) {
	  ...
	  for(fid = 0; fid < selecttab[s].nfds; fid++) {
	    ...
	    if(dmap_driver_match(proc_e, maj)) {
			select_return(&selecttab[s], EDVSAGAIN);
	    }
	  }
	}
