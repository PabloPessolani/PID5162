...
SVRDEBUG( MSG3_FORMAT, MSG3_FIELDS(m_ptr));
   		...
   		if(who_p < -(vm_ptr->vm_nr_tasks) || who_p >= (vm_ptr->vm_nr_procs))
...
#else // DEMONIZE	
#define VMID 		0
		SVRDEBUG("Before bind\n");
		rcode = mnx_bind(VMID,fs_ep);
		SVRDEBUG("After bind rcode=%d\n", rcode);
		
			/* Register into SYSTASK (as an autofork) */
		SVRDEBUG("Register FATFS into SYSTASK pm_lpid=%d\n",fs_lpid);
		fs_ep = sys_bindproc(fs_ep, fs_lpid, REPLICA_BIND);
		if(fs_ep < 0) ERROR_EXIT(fs_ep);
	
		rcode = sys_rsetpname(fs_ep, "FatFS", local_nodeid);
	...
	SVRDEBUG("Get fs_ep info from SYSTASK\n");
...
// 	fproc = malloc((vm_ptr->vm_nr_procs)*sizeof(fproc_t));
	posix_memalign( (void**) &fproc, getpagesize(), (vm_ptr->vm_nr_procs)*sizeof(fproc_t));
	...
	posix_memalign( (void**) &mproc, getpagesize(), (vm_ptr->vm_nr_procs)*sizeof(mproc_t));
