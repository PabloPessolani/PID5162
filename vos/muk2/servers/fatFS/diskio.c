...
int RDISK_disk_initialize(FF_BYTE pdrv)
{
	...
	if ((status=dev_open(root_dev, fs_ep, R_BIT|W_BIT)) != OK)
...
int RDISK_disk_read(FF_BYTE* buff,  FF_DWORD sector, FF_UINT count)
{
	...
	ret = dev_io(DEV_READ, root_dev, fs_ep, buff, sector * SECTOR_SIZE, count * SECTOR_SIZE, O_RDWR);//Ver lo de las flags
...
int RDISK_disk_write(FF_BYTE* buff,  FF_DWORD sector, FF_UINT count)
{
	...
	ret = dev_io(DEV_WRITE, root_dev, fs_ep, buff, sector * SECTOR_SIZE, count * SECTOR_SIZE, O_RDWR);//Ver lo de las flags
