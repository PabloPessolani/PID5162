...
/* Perform the advisory locking required by POSIX. */
 ...
 //  r = sys_datacopy(who_e, (vir_bytes) user_flock,
	// fs_ep, (vir_bytes) &flock, (phys_bytes) sizeof(flock));
  r = mnx_vcopy(who_e, user_flock, fs_ep, &flock, sizeof(flock));    
  ...
  if (req == F_GETLK) {
	...
	// r = sys_datacopy(fs_ep, (vir_bytes) &flock,
	// 	who_e, (vir_bytes) user_flock, (phys_bytes) sizeof(flock));
 	r = mnx_vcopy(fs_ep, &flock, who_e, user_flock, sizeof(flock));	
	return(r);
  }
 ...
 */
  ...
  for (fptr = &fproc[INIT_PROC_NR + 1]; fptr < &fproc[vm_ptr->vm_nr_procs]; fptr++){
