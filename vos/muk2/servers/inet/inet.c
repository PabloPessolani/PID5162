...
int main(int argc,  char *argv[])
{
	...
	if ((this_proc = muk_getep(getpid())) < 0)
	...
	while(TRUE){
    	ret = muk_receive(ANY, (long) m_ptr);
		SVRDEBUG("" MSG1_FORMAT, MSG1_FIELDS(m_ptr));
		m_ptr->m_type = OK;
    	ret = muk_send(m_ptr->m_source, (long) m_ptr);		
	}
...
//	nw_test_icmp();
	...
	while (TRUE){
		...
		SVRDEBUG("muk_receive\n");
		rcode = muk_receive (ANY, &mq->mq_mess);
		if (rcode<0){
...
void inet_init(void)
{
	...
	do { 
		...
		SVRDEBUG("INET: muk_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EDVSTIMEDOUT) {
			SVRDEBUG("INET: muk_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
// 	Source MAC - It will be  overwritten by interface task
	...
	rcode = muk_sendrec(ETH_PROC_NR, m_ptr);
	if(rcode != 0){
		ERROR_PRINT(rcode);
	}
