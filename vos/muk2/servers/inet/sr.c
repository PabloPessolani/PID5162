...
void sr_rec(mq_t *m)
{
	...
	switch (m->mq_mess.m_type)	{
		...
		case DEV_CANCEL:
			...
			assert(result == OK || result == EDVSINTR);
	//		send_reply = (result == EDVSINTR); MODIFICADO POR PAP
			send_reply = 1;	
			free_mess= 1;
			m->mq_mess.m_type= 0;
			break;
...
int sr_open(message *m)
{
	...
	if (minor<0 || minor>FD_NR)	{
		DBLOCK(1, printf("replying EDVSINVAL\n"));
		return EDVSINVAL;
	}
	...
	switch(m->mq_mess.m_type)	{
		...
		case DEV_IOCTL3:
			...
			if (size>MAX_IOCTL_S)
			{
				...
				DBLOCK(1, printf("replying EDVSINVAL\n"));
				r= sr_put_userdata(sr_fd-sr_fd_table, EDVSINVAL, 
					NULL, 1);
				assert(r == OK);
				assert(sr_fd->srf_flags & first_flag);
				sr_fd->srf_flags &= ~first_flag;
...
message *m;
{
	...
	if (mq != NULL)	{
		...
		result= muk_send(mq->mq_mess.m_source, &mq->mq_mess);
		if (result != OK)
...
void sr_reply_(mq_t *mq,int status,int is_revive)
{
	...
	if (is_revive)	{
		muk_notify(mq->mq_mess.m_source);
		result= EDVSLOCKED;
	} else
		result= muk_send(mq->mq_mess.m_source, mp);

	if (result == EDVSLOCKED && is_revive)	{
		mq->mq_next= NULL;
		if (repl_queue)
...
void sr_select_res(int fd, unsigned ops)
{
	...
	muk_notify(sr_fd->srf_select_proc);
}
...
//		vir_cp_req[i].dst.segment = D;
		...
		if (i == CPVEC_NR || acc == NULL)	{
			...
			if (muk_sendrec(SYSTASK(local_nodeid), &mess) <0)
				ip_panic(("unable to muk_sendrec("));
			if (mess.m_type <0)	{
...
int sr_repl_queue(int proc, int ref, int operation)
{
	...
	for (m= repl_queue; m;)	{
		...
		result= muk_send(m->mq_mess.m_source, &m->mq_mess);
		if (result != OK)
	...
	repl_queue= NULL;
	if (m_cancel) {
		result= muk_send(m_cancel->mq_mess.m_source, &m_cancel->mq_mess);
		if (result != OK)
			ip_panic(("unable to send: %d", result));
		mq_free(m_cancel);
		return 1;
