...
int main ( int argc, char *argv[] )
{
	...
	vmid = atoi(argv[1]);
	if( vmid < 0 || vmid >= NR_VMS) ERROR_EXIT(EDVSRANGE);
...
SVRDEBUG("pm_call_nr=%d result=%d\n", pm_call_nr, result);
...
void get_work(void)
{
	...
	while( TRUE) {
		...
		if ( (ret=mnx_receive(ANY, &pm_m_in)) != OK) ERROR_EXIT(ret);
		who_e = pm_m_in.m_source;	/* who sent the message */
		pm_call_nr = pm_m_in.m_type;	/* system call number */
SVRDEBUG("Request received from who_e=%d, pm_call_nr=%d\n", who_e, pm_call_nr);

		if( (who_e == CLOCK) && (pm_call_nr & NOTIFY_MESSAGE) ) return;

		if(( ret = pm_isokendpt(who_e, &who_p)) != OK) {
SVRDEBUG("BAD ENDPOINT endpoint=%d, pm_call_nr=%d\n", who_e, pm_call_nr);
//			pm_m_in.m_type = EDVSENDPOINT;
//			ret = mnx_send(who_e, &pm_m_in);
			continue;
		} 
		...
		*/
		kp = &kproc[(who_p < 0 ? pm_ep : who_p)+vm_ptr->vm_nr_tasks];
		mp = &mproc[who_p < 0 ? pm_ep : who_p];
...
SVRDEBUG("Get the VM pm_info from SYSTASK\n");
	sys_getmachine(&vmu);
	if(rcode) ERROR_EXIT(rcode);
	vm_ptr = &vmu;
