		...
		iovec_dat_t *iov_addr, vir_bytes offset,vir_bytes count)
{
	...
	while (count > 0){
		...
		/*
		  ...
		  */
		if ( (r=muk_vcopy( SELF, nic_addr, iov_addr->iod_proc_nr, 
	...
	vir_bytes offset, int nic_addr, vir_bytes count)
{
	...
	while (count > 0)  {
		...
		if ( (rcode = muk_vcopy(iov_addr->iod_proc_nr, iov_addr->iod_iovec[i].iov_addr + offset,
				SELF, nic_addr, count )) != OK ) {
			TASKDEBUG("muk_vcopy rcode=%d\n", rcode);
			ERROR_EXIT(rcode);
		}   
...
static void eth_receive(void *arg)
{
	...
	while(1) {
		...
		if(bm_ports > 0) { /* Handle incoming packet. */
			muk_hdw_notify(eth_ptr->p_endpoint, bm_ports);
		} 
	}
}
...
static void  eth_init(void)
{
	...
	do { 
		...
		TASKDEBUG("ETH: muk_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EDVSTIMEDOUT) {
			TASKDEBUG("ETH: muk_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
void reply(eth_card_t *ec_ptr, int err, int may_block)
{
	...
	rcode = muk_send(ec_ptr->client, &reply);
...
//	cps = sys_vircopy(SELF, (void *) loc_addr, user_proc, (void *) user_addr, count);
	cps = muk_vcopy(SELF, (void *) loc_addr, user_proc, (void *) user_addr, count);
	if (cps != OK) {
...
int main(int argc, char *argv[] )
{
	...
	while(TRUE){
		TASKDEBUG("waiting on muk_receive_T\n");
		rcode = muk_receive_T(ANY, mi_ptr, RECEIVE_TIMEOUT);
		TASKDEBUG("rcode=%d\n", rcode);
