...
/*!!!!!!!!!!!!!! VERIFICAR QUE EL M_SOURCE/REQUESTER TENGA PRIVILEGIOS !!!!!!!!!!!!!!!!!!!!*/
	...
	src_ptr = get_task(src_nr);
	proc_ptr = src_ptr->p_proc;
	MUKDEBUG("SRC " PROC_USR_FORMAT,PROC_USR_FIELDS(proc_ptr));
			
	dst_ptr = get_task(dst_nr);
	proc_ptr = dst_ptr->p_proc;
	MUKDEBUG("SRC " PROC_USR_FORMAT,PROC_USR_FIELDS(proc_ptr));
	
	if( src_ptr->p_proc->p_rts_flags == SLOT_FREE) ERROR_RETURN(EDVSSRCDIED);
	if( dst_ptr->p_proc->p_rts_flags == SLOT_FREE) ERROR_RETURN(EDVSDSTDIED);
