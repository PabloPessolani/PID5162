...
MUKDEBUG("new_sec=%lu new_usec=%lu\n", p_new->tv_sec,p_new->tv_usec);  
...
// MUKDEBUG("clockTicks=%d next_timeout=%ld realtime=%ld \n",clockTicks, next_timeout, realtime );
...
MUKDEBUG("new_sec=%lu new_usec=%lu\n", new_interval.tv_sec,new_interval.tv_usec); 
	...
	next_timeout = (clock_timers == NULL) ? 
			TMR_NEVER : clock_timers->dvk_tmr_exp_time;
...
// MUKDEBUG("tv_sec=%llu tv_nsec=%llu uptime=%lld \n", (unsigned long long)t.tv_sec, 
...
MUKDEBUG("exp_time=%d\n",exp_time);
  ...
  next_timeout = clock_timers->dvk_tmr_exp_time;
MUKDEBUG("realtime=%d exp_time=%d next_timeout=%ld \n",(get_uptime()-init_time), exp_time, next_timeout);
