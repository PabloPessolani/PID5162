 ...
 *				st_vcopy					     *
 *===========================================================================*/
int st_vcopy(register message *m_ptr)
{
	int nr_req;
	int bytes;
	int i,rcode;
	...
	MUKDEBUG("VCP_VEC_SIZE=%d, VCP_VEC_ADDR=%X, VCP_NR_OK=%d\n",		
				m_ptr->VCP_VEC_SIZE,
				m_ptr->VCP_VEC_ADDR,
				m_ptr->CP_DST_ENDPT);
	...
	/* copy request vector  */
	MUK_vcopy(rcode, sys_who_e, m_ptr->VCP_VEC_ADDR, 
						SELF, cp_req ,bytes);
	...
	for (i=0; i<nr_req; i++) {
		...
		MUKDEBUG(VIRCP_FORMAT, VIRCP_FIELDS(req));
					
		MUK_vcopy(rcode , req->src.proc_nr_e, req->src.offset,
							req->dst.proc_nr_e, req->dst.offset,
							req->count);
		if(rcode < 0) ERROR_RETURN(rcode);
		m_ptr->VCP_NR_OK ++;
	}
	MUKDEBUG("VCP_NR_OK=%d\n",	m_ptr->VCP_NR_OK);	
	return(OK);
}
