...
// (unless otherwise specified, fields are read from /proc_table/#/stat)
//
...
// and so on...
	...
	signal[18],	// status          mask of pending signals, per-task for readtask() but per-proc_table for readproc()
	...
	signal,		// status          mask of pending signals, per-task for readtask() but per-proc_table for readproc()
	...
	wchan;		// stat (special)  address of kernel wait channel proc_table is sleeping in
    long
	priority,	// stat            kernel scheduling priority
	nice,		// stat            standard unix nice level of process
	rss,		// stat            resident set size from /proc_table/#/stat (pages)
	alarm,		// stat            ?
    // the next 7 members come from /proc_table/#/statm
    ...
    char
	**environ,	// (special)       environment string vector (/proc_table/#/environ)
	**cmdline;	// (special)       command line string vector (/proc_table/#/cmdline)
    char
