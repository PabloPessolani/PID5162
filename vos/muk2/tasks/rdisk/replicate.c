...
int init_replicate(void)
{
	...
	MUKDEBUG("Spread library version is %d.%d.%d\n", mver, miver, pver);
#else
    MUKDEBUG("Spread library version is %1.2f\n", SP_version() );
#endif
	/*------------------------------------------------------------------------------------
	...
	*--------------------------------------------------------------------------------------*/
	...
	MUKDEBUG("User %s: connected to %s with private group %s\n",
...
int replica_loop(int *mtype, char *source)
{
	...
	MUKDEBUG("sender=%s Private_group=%s dc_name=%s service_type=%d\n"
	...
	MUKDEBUG("dynup_flag:%d - DO_DYNUPDATES:%d\n", dynup_flag, DO_DYNUPDATES);
	...
	if( Is_regular_mess( service_type ) )	{
		mess_in[ret] = 0;
		if     ( Is_unreliable_mess( service_type ) ) {MUKDEBUG("received UNRELIABLE \n ");}
		else if( Is_reliable_mess(   service_type ) ) {MUKDEBUG("received RELIABLE \n");}
		else if( Is_causal_mess(       service_type ) ) {MUKDEBUG("received CAUSAL \n");}
		else if( Is_agreed_mess(       service_type ) ) {MUKDEBUG("received AGREED \n");}
		else if( Is_safe_mess(   service_type ) || Is_fifo_mess(       service_type ) ) {
			MUKDEBUG("message from %s, of type %d, (endian %d) to %d groups (%d bytes)\n",
			...
			*----------------------------------------------------------------------------------------------------*/
			if( mess_type == DEV_WRITE ) {
				MUKDEBUG(MSG2_FORMAT, MSG2_FIELDS(msg_ptr));
			...
			*----------------------------------------------------------------------------------------------------*/
				...
				*mtype = mess_type;
				...
				MUKDEBUG("RADAR Message, ignored..\n");
				*mtype = mess_type;
			} else {
				MUKDEBUG("Unknown message type %X\n", mess_type);
				MUKDEBUG(MSG2_FORMAT, MSG2_FIELDS(msg_ptr));
				*mtype = mess_type;
				ret = OK;
			}
        ...
        if (ret < 0) {
			MUKDEBUG("BUG: membership message does not have valid body\n");
           	SP_error( ret );
		...
		if  ( Is_reg_memb_mess( service_type ) ) {
			MUKDEBUG("Received REGULAR membership for group %s with %d members, where I am member %d:\n",
...
//					MUKDEBUG("grp id is %d %d %d\n",memb_info.gid.id[0], memb_info.gid.id[1], memb_info.gid.id[2] );
		...
		if( Is_caused_join_mess( service_type ) )	{
			...
			*----------------------------------------------------------------------------------------------------*/
			MUKDEBUG("Due to the JOIN of %s service_type=%d\n", 
			...
			if ( strncmp(memb_info.changed_member, "#RDISK",6) == 0) { // RDISK member 		
				...
				MUKDEBUG("JOIN - nr_nodes=%d\n", nr_nodes); 
				ret = sp_join(mbr);
				MUKDEBUG("JOIN end - nr_nodes=%d\n", nr_nodes); 
			}
			...
			*----------------------------------------------------------------------------------------------------*/
			MUKDEBUG("Due to the LEAVE or DISCONNECT of %s\n", 
				memb_info.changed_member );
			if ( strncmp(memb_info.changed_member, "#RDISK",6) == 0) { // RDISK member 		
				mbr = get_nodeid("RDISK", (char *)  memb_info.changed_member);
				nr_nodes = num_groups - nr_radar;
				MUKDEBUG("LEAVE - nr_nodes=%d\n", nr_nodes); 
				ret = sp_disconnect(mbr);
				MUKDEBUG("LEAVE end - nr_nodes=%d\n", nr_nodes); 
				if( local_nodeid == primary_mbr)
					mcast_radar_info();
			}
			...
			*----------------------------------------------------------------------------------------------------*/
			MUKDEBUG("Due to NETWORK change with %u VS sets\n", memb_info.num_vs_sets);
            		num_vs_sets = SP_get_vs_sets_info( mess_in, 
									&vssets[0], MAX_VSSETS, &my_vsset_index );
            if (num_vs_sets < 0) {
				MUKDEBUG("BUG: membership message has more then %d vs sets. Recompile with larger MAX_VSSETS\n", MAX_VSSETS);
				SP_error( num_vs_sets );
				// pthread_mutex_unlock(&rd_mutex);
				MTX_UNLOCK(bk_mutex);
               	ERROR_TSK_EXIT( num_vs_sets );
			}
            if (num_vs_sets == 0) {
				MUKDEBUG("BUG: membership message has %d vs_sets\n", 
					num_vs_sets);
				SP_error( num_vs_sets );
				// pthread_mutex_unlock(&rd_mutex);
				MTX_UNLOCK(bk_mutex);
               	ERROR_TSK_EXIT( EDVSGENERIC );
			}

			bm_nodes = 0;
			nr_nodes = 0;
            for( i = 0; i < num_vs_sets; i++ )  {
				MUKDEBUG("%s VS set %d has %u members:\n",
					(i  == my_vsset_index) ?("LOCAL") : ("OTHER"), 
						i, vssets[i].num_members );
               	ret = SP_get_vs_set_members(mess_in, &vssets[i], members, MAX_MEMBERS);
               	if (ret < 0) {
					MUKDEBUG("VS Set has more then %d members. Recompile with larger MAX_MEMBERS\n", MAX_MEMBERS);
					SP_error( ret );
					// pthread_mutex_unlock(&rd_mutex);
					MTX_UNLOCK(bk_mutex);
                   	ERROR_TSK_EXIT( ret);
				...
				* get the bitmap of current members
				...
				MUKDEBUG("old bm_sync=%X bm_nodes=%X primary_mbr=%d nr_radar=%d bm_radar=%X\n",
			...
			if( bm_sync > bm_nodes) {		/* a NETWORK PARTITION has occurred 	*/
				...
				if (bm_sync < bm_nodes) {	/* a NETWORK MERGE has occurred 		*/
					...
					MUKDEBUG("NETWORK CHANGE with no changed members (may be RADAR member) !! ");
				}
			}
			if( local_nodeid == primary_mbr)
				mcast_radar_info();
		}else if( Is_transition_mess(   service_type ) ) {
			MUKDEBUG("received TRANSITIONAL membership for group %s\n", sender );
			if( Is_caused_leave_mess( service_type ) ){
				MUKDEBUG("received membership message that left group %s\n", sender );
			}else {
				MUKDEBUG("received incorrecty membership message of type 0x%x\n", service_type );
			}
		} else if ( Is_reject_mess( service_type ) )      {
			MUKDEBUG("REJECTED message from %s, of servicetype 0x%x messtype %d, (endian %d) to %d groups \n(%d bytes): %s\n",
				sender, service_type, mess_type, endian_mismatch, num_groups, ret, mess_in );
		}else {
			MUKDEBUG("received message of unknown message type 0x%x with ret %d\n", service_type, ret);
		}
	}
	// pthread_mutex_unlock(&rd_mutex);
...
{
	int rcode; 
	MUKDEBUG("new_member=%d primary_mbr=%d nr_nodes=%d\n", 
		new_mbr, primary_mbr, nr_nodes);
	if( nr_nodes < 0 || nr_nodes >= dc_ptr->dc_nr_nodes){
		MUKDEBUG("nr_nodes=%d dc_ptr->dc_nr_nodes=%d\n", nr_nodes, dc_ptr->dc_nr_nodes);
		ERROR_RETURN(EDVSINVAL);
		}
		
	if( TEST_BIT(bm_radar, new_mbr) != 0){
		MUKDEBUG("new_mbr=%d is in the same node as RADAR (bm_radar=%X)\n", new_mbr, bm_radar);
		ERROR_TSK_EXIT(EDVSBADNODEID);
	}
	...
	if( new_mbr == local_nodeid){		/*  My own JOIN message	 */
		if (nr_nodes == 1){ 			/* I am a LONELY member  */
			...
			synchronized = TRUE;
			MUKDEBUG("synchronized=%d'n", synchronized);
			
			if ( ! replica_updated(local_nodeid)) {
				MUKDEBUG("I am a BACKUP member: start the primary FIRST\n");
				rcode = EDVSPRIMARY;
				SP_error(rcode);
				// pthread_mutex_unlock(&rd_mutex);		
				MTX_UNLOCK(bk_mutex);
			...
			MUKDEBUG("PRIMARY_MBR=%d\n", primary_mbr);
			MUKDEBUG("Wake up rdisk: new_mbr=%d\n", new_mbr);
			...
			if (local_nodeid != primary_mbr){/*SLAVE*/
				...
				MUKDEBUG("Initializing SLAVE COPY THREAD\n");
			
				slave_ac = TRUE;
				sync_pr = TRUE;
				MUKDEBUG("slave_ac=%d sync_pr=%d\n", slave_ac, sync_pr);
				MUKDEBUG("dynup_flag%d\n", dynup_flag);
				...
				MUKDEBUG("rcode - pthread_join-slave\n", rcode);
				...
				if( rcode){
					sync_pr = FALSE;
					/*check open device*/
					MUKDEBUG("Sync backpup error - chek open device\n");
		...
		if (primary_mbr == local_nodeid){ 	/* I am the first init member 	*/
			...
			// }
			MUKDEBUG("Initializing MASTER COPY THREAD\n");
			master_ac = TRUE;
			sync_pr = TRUE;
			MUKDEBUG("master_ac=%d sync_pr=%d\n", master_ac, sync_pr);
			MUKDEBUG("dynup_flag%d\n", dynup_flag);
			...
			MUKDEBUG("Starting MASTER COPY\n");
			MUKDEBUG("new_mbr %d, %d\n", &new_mbr, new_mbr);
			...
			master_ac = FALSE;
			MUKDEBUG("master_ac=%d\n", master_ac);
			MUKDEBUG("rcode - pthread_join-master\n", rcode);
			if( rcode){
				master_ac = FALSE;
				sync_pr = FALSE;
				MTX_UNLOCK(bk_mutex);
...
int sp_disconnect(int  disc_mbr)
{
	...
	MUKDEBUG("CLR_BIT bm_nodes=%d, disc_mbr=%d\n",bm_nodes, disc_mbr);
	...
	MUKDEBUG("bm_sync=%d, disc_mbr=%d, nr_sync=%d\n",bm_sync, disc_mbr, nr_sync);
	...
	MUKDEBUG("primary_mbr=%d, disc_mbr=%d\n",primary_mbr, disc_mbr);
	MUKDEBUG("bm_sync=%d, disc_mbr=%d, nr_sync=%d\n",bm_sync, disc_mbr, nr_sync);
			...
			for( i = 0; i < NR_NODES; i++) {
				if(TEST_BIT(bm_nodes, i)) {
					if( i == local_nodeid){ /* I am the new primary */
						primary_mbr = local_nodeid;
						MUKDEBUG("PRIMARY_MBR=%d\n", primary_mbr);
						MUKDEBUG("Wake up rdisk: local_nodeid=%d\n", local_nodeid);
	...
	MUKDEBUG("disc_mbr=%d nr_nodes=%d\n",	disc_mbr, nr_nodes);
	
	
	MUKDEBUG("primary_mbr=%d nr_sync=%d\n", primary_mbr, nr_sync);
	MUKDEBUG("bm_nodes=%X bm_sync=%X bm_acks=%X\n", bm_nodes,bm_sync,bm_acks);
...
int sp_net_partition(void)
{
	...
	if( TEST_BIT(bm_sync, first_act_mbr)){
		...
		if(first_act_mbr == NO_FIRST_INIT_MBR){
			MUKDEBUG("Can't find primary_mbr of this partition %d\n",primary_mbr);
	...
	for( i = dc_ptr->dc_nr_sysprocs; i < (dc_ptr->dc_nr_tasks + dc_ptr->dc_nr_procs);i++) {
		...
		if( proc[i].p_rts_flags == SLOT_FREE) {
			if ( (TEST_BIT(proc[i].p_rts_flags, BIT_DONATING)) &&
				...
				MUKDEBUG("Restoring slot %d from uncompleted donation after a network partition\n",i);
			}
		}
	...
	* Alloc memory for temporal  PST
	...
	for( i = dc_ptr->dc_nr_sysprocs; i < (dcu.dc_nr_procs+dcu.dc_nr_tasks); i++) {
		...
		if( !TEST_BIT(bm_sync, slot[i].s_owner)) { /* the owner is not on this partition */
			MUKDEBUG("slot reserved %d for owner=%d\n", i, slot_part[i].s_owner);
	...
	MUKDEBUG("Send the partition's PST to all members \n");
 ...
 int rep_dev_write(SP_message *sp_ptr){
		...
		if (( slave_ac == TRUE) || ( sync_pr == TRUE)) {
			...
			if ((( r_type == DEV_CFULL ) || ( r_type == DEV_CMD5)) && ( dynup_flag == DO_DYNUPDATES)) { /*sync: only updates*/
				...
				if( primary_mbr == local_nodeid){
					...
					MUKDEBUG("BACKUP sync\n");					
					
					MUKDEBUG("Transfer block: %u\n", r_trblksize);
					...
					if ( sp_nrblock < s_nrblock ){ /*updates: device in sync*/
						...
						if ( sp_ptr->buf.flag_buff == COMP ){
							...
							if ( msg_ptr->COUNT == sp_ptr->buf.buffer_size) {
								...
								MUKDEBUG("dev: %d, nr_update: %u\n", msg_ptr->DEVICE, sumdevvec[msg_ptr->DEVICE].nr_updated);
							} 
							else{
								MUKDEBUG("ERROR. Bytes decompress not equal Bytes original\n");
								MTX_UNLOCK(write_mutex);
								ERROR_TSK_EXIT( EDVSPACKSIZE );
							}	
												
						}
						else{		
							MUKDEBUG("DATA BUFFER UNCOMPRESS\n");
		
							if ( (pwrite(devvec[msg_ptr->DEVICE].img_p, sp_ptr->buf.buffer_data, msg_ptr->COUNT, msg_ptr->POSITION)) < 0 ){
								rcode = errno;
								MUKDEBUG("devvec[msg_ptr->DEVICE].img_p= %d, sp_ptr->buf.buffer_data= %s, msg_ptr->COUNT= %u, msg_ptr->POSITION=%X\n",
				...
				MUKDEBUG("SERÍA POR LA COPIA FULL, VER???\n"); 
				return(OK);
	...
	if (( synchronized == TRUE) && ( sync_pr == 0 ) && ( (master_ac == 0) || (slave_ac == 0) )){
		...
		if ( msg_ptr->m_source != primary_mbr){
			MUKDEBUG("FAKE PRIMARY member=%d\n", msg_ptr->m_source);
			return(OK);
		}
		
		MUKDEBUG("bm_acks=%d, bm_sync=%d \n", bm_acks, bm_sync); 	
		bm_acks = bm_sync;
		CLR_BIT(bm_acks,primary_mbr);
		MUKDEBUG("CLR_BIT -bm_acks=%d\n", bm_acks); 	
		MUKDEBUG("primary_mbr=%d, local_nodeid= %d\n", primary_mbr, local_nodeid); 	
		...
		MUKDEBUG("iovec1.iov_addr= %X\n", iovec1.iov_addr);
		MUKDEBUG("iovec1.iov_size= %d\n", iovec1.iov_size);
		
		MUKDEBUG("File descriptor image= %d\n", devvec[msg_ptr->DEVICE].img_p);
		MUKDEBUG("(receive) msg_ptr->POSITION %X\n", msg_ptr->POSITION);	
		MUKDEBUG("sp_ptr->buf.flag_buff =%d\n", sp_ptr->buf.flag_buff);
		MUKDEBUG("buffer: %s\n", sp_ptr->buf.buffer_data);			
		...
		if ( sp_ptr->buf.flag_buff == COMP ){
			...
			MUKDEBUG("DATA BUFFER UNCOMPRESS\n");
			MUKDEBUG("msg_ptr->DEVICE].img_p=%d, sp_ptr->buf.buffer_data= %s, msg_ptr->COUNT= %d, msg_ptr->POSITION= %X\n", 
					devvec[msg_ptr->DEVICE].img_p, 
					sp_ptr->buf.buffer_data, 
					msg_ptr->COUNT, 
					msg_ptr->POSITION);
 ...
 *				rep_task_reply								     	*
 ===========================================================================*/
int rep_task_reply( message *rd_mptr)
{
	...
	if( !TEST_BIT(bm_sync, rd_mptr->m_source)){
		MUKDEBUG("FAKE backup member=%d (bm_sync=%X)\n", 
			rd_mptr->m_source, bm_sync );
		return(OK);
	}

	CLR_BIT(bm_acks, rd_mptr->m_source);
	MUKDEBUG("rd_mptr->m_source=%d\n",rd_mptr->m_source); 
	
  	if( primary_mbr == local_nodeid){
		if(bm_acks == 0) {	/* all ACKs received */
			MUKDEBUG("ALL ACKS received\n"); 
			// pthread_cond_signal(&update_barrier);	/* Wakeup RDISK 		*/
			COND_SIGNAL(update_barrier);
			}
	}
	MUKDEBUG("termina la respuesta\n"); 
	return(OK);
}
 ...
 *				rep_dev_open								     	*
 ===========================================================================*/
int rep_dev_open( message *rd_mptr)
{
	int rcode;
	unsigned *localbuff;
	message msg;
	...
	if ( rd_mptr->m_source != primary_mbr){
		MUKDEBUG("FAKE PRIMARY member=%d\n", rd_mptr->m_source);
		return(OK);
	}
	...
	if( primary_mbr == local_nodeid){
		MUKDEBUG("sync proccesing - multicast DEV_OPEN REPLY to %d rcode=%d\n", local_nodeid ,rcode);
		MUKDEBUG("master_ac=%d sync_pr=%d\n", master_ac, sync_pr);
		if (( master_ac == TRUE) || ( sync_pr == TRUE)) {
			MUKDEBUG("master_ac=%d sync_pr=%d\n", master_ac, sync_pr);
			msg.m_source= local_nodeid;	
				MUKDEBUG("msg.m_source= %d\n", msg.m_source);	
				msg.m_type 	= MOLTASK_REPLY;
				msg.m2_i1	= rd_mptr->DEVICE;
				msg.m2_i2	= DEV_OPEN;
				msg.m2_i3	= rcode;
				rcode = SP_multicast (sysmbox, SAFE_MESS, (char *) rdisk_group,  
						MOLTASK_REPLY, sizeof(message), (char *) &msg); 
	...
	rcode = m_do_open(rd_dtab, rd_mptr); // modificado pap antes era &rd_dtab
	MUKDEBUG("rcode=%d\n");
	if(rcode < 0){
		MTX_UNLOCK(bk_mutex);
		ERROR_RETURN(rcode);
...
int rep_dev_close( message *sp_ptr){
		MUKDEBUG(MSG2_FORMAT, MSG2_FIELDS(sp_ptr));
		return(OK);
}
...
int mc_status_info(	message  *sp_ptr)
{
	...
	if (nr_nodes != sp_ptr->m2_i1){
		MUKDEBUG("Received nr_nodes=%d don't match local nr_nodes=%d\n"
...
int mc_synchronized(  message  *sp_ptr)
{
	...
	if ( sp_ptr->m_source == local_nodeid) {
		...
		MUKDEBUG("synchronized=%d\n", synchronized);
		sync_pr = FALSE; //MARIE
		MUKDEBUG("Sync proccesing end - sync_pr=%d\n", sync_pr);
		return(OK);
	}
	/* ESTO NO SE SI ESTA BIEN 	
...
int get_dcid(char *grp_name, char *mbr_string)
{
	...
	MUKDEBUG("grp_name=%s mbr_string=%s len=%d\n", grp_name,mbr_string, len);
...
int send_status_info(void)
{
	...
	MUKDEBUG("local_nodeid=%d, nr_nodes=%d, nr_sync=%d\n", local_nodeid, nr_nodes, nr_sync);
