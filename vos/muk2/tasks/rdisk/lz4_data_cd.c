...
int compress_payload( data_desc_t *data_ptr )
{
	...
	*/
	...
	MUKDEBUG("comp_len: %d - data_ptr->td_comp_pl: %X - data_ptr->td_maxCsize: %d - data_ptr->td_payload: %X - data_ptr->td_td_maxRsize: %d\n",
	...
	MUKDEBUG("COMPRESS_DATA: buffCsize=%d comp_len=%d\n", data_ptr->td_maxCsize, comp_len);
	
	MUKDEBUG("END_compress_payload\n");	
	return(comp_len);
}

void stop_compression( data_desc_t *data_ptr) 
{
	MUKDEBUG("stop_compression\n");
	
	free(data_ptr->td_comp_pl);
	MUKDEBUG("free dst_buffer(td_comp_ln)\n");
}
...
void init_decompression( data_desc_t *data_ptr) 
{
	...
	*/
	...
	MUKDEBUG("END_init_decompression\n");
}
...
int decompress_payload( data_desc_t *data_ptr)
{
	...
	MUKDEBUG("Decompress_payload: INPUT td_maxRsize=%d td_maxCsize=%d \n", 
		data_ptr->td_maxRsize,data_ptr->td_maxCsize );

	comp_len = data_ptr->td_maxCsize;
	raw_len  = data_ptr->td_maxRsize;
	
	MUKDEBUG("data_ptr->td_payload: %X - data_ptr->td_comp_pl: %X\n", data_ptr->td_payload,data_ptr->td_comp_pl);
	MUKDEBUG("data_ptr->td_payload: %s - data_ptr->td_comp_pl: %s\n", data_ptr->td_payload,data_ptr->td_comp_pl);
	MUKDEBUG("comp_len=%d raw_len=%d \n", comp_len, raw_len );
	MUKDEBUG("data_ptr->td_lz4Dctx=%d \n", data_ptr->td_lz4Dctx );
	...
	*/
	...
	if (LZ4F_isError(lz4_rcode)) {
		MUKDEBUG("LZ4F_decompress failed: error %zu, %s\n", lz4_rcode, LZ4F_getErrorName(lz4_rcode));
		exit(lz4_rcode);
	}
	
	MUKDEBUG("END_decompress: OUTPUT raw_len=%d comp_len=%d\n", raw_len, comp_len);
	return(raw_len);					  
...
void stop_decompression( data_desc_t *data_ptr) 
{
	...
	if (LZ4F_isError(lz4_rcode)) {
		...
		ERROR_TSK_EXIT(lz4_rcode);
	}
	
	free(data_ptr->td_payload);
	MUKDEBUG("free dst_buffer(td_payload)\n");
	
	MUKDEBUG("END_Stop_decompression\n");
}
...
if ( flag_in == UNCOMP ){
	MUKDEBUG("COMPRESS DATA, UNCOMP =%d\n", UNCOMP);
	
	rdesc.td_payload = in_buffer;
	MUKDEBUG("rdesc.td_payload: %X\n", rdesc.td_payload);
	MUKDEBUG("rdesc.td_payload: %s\n", rdesc.td_payload);
	rdesc.td_maxRsize = inbuffer_size;
	MUKDEBUG("rdesc.td_maxRsize: %d\n", rdesc.td_maxRsize);
		
	init_compression(&rdesc);			
		
	len = compress_payload(&rdesc);

	MUKDEBUG("rdesc.td_comp_pl: %X\n", rdesc.td_comp_pl);
	MUKDEBUG("rdesc.td_comp_pl: %s\n", rdesc.td_comp_pl);
	MUKDEBUG("Bytes compress_payload: %d\n", len);
	
	msg_lz4cd.buf.flag_buff = COMP;
	MUKDEBUG("msg_sp->buf.flag_buff: %d\n", msg_lz4cd.buf.flag_buff);
	msg_lz4cd.buf.buffer_size = len;
	MUKDEBUG("msg_sp->buf.buffer_size: %d\n", msg_lz4cd.buf.buffer_size);
	memcpy(msg_lz4cd.buf.buffer_data, rdesc.td_comp_pl, len); 
	MUKDEBUG("msg_sp->buf.buffer_data: %S\n", msg_lz4cd.buf.buffer_data);
	
	stop_compression(&rdesc);
			
	MUKDEBUG("END - COMPRESS DATA\n");
}
else
{
	if ( flag_in == COMP ){
		MUKDEBUG("UNCOMPRESS DATA\n");
		
		rdesc.td_comp_pl = in_buffer;
		MUKDEBUG("rdesc.td_comp_pl: %X\n", rdesc.td_comp_pl);
		MUKDEBUG("rdesc.td_comp_pl: %s\n", rdesc.td_comp_pl);
		rdesc.td_maxCsize = inbuffer_size;
		MUKDEBUG("rdesc.td_maxCsize: %d\n", rdesc.td_maxCsize);
