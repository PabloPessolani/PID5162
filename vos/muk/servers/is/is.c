  ...
  /* Main loop - get work and do it, forever. */  
	...
	while (TRUE) {              
		...
		if( TEST_BIT( dump_type, DMP_SYS_DC)){
			...
			rcode = dvk_sendrec(SYSTASK(local_nodeid), m_ptr);
...
//	if(is_ep < 0) ERROR_PT_EXIT(is_ep);
