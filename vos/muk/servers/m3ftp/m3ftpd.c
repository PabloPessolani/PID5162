...
int  main_ftpd ( int argc, char *argv[] )
{
	...
	while(TRUE){
		...
		do { 
			...
			MUKDEBUG("M3FTPD: dvk_receive_T  rcode=%d\n", rcode);
			if (rcode == EDVSTIMEDOUT) {
				MUKDEBUG("M3FTPD: dvk_receive_T TIMEOUT\n");
				continue ;
			}else if( rcode < 0) 
		...
		switch(ftp_mptr->FTPOPER){
			case FTP_GET:
				...
				while( (rlen = mol_read(ftpd_fd, ftp_data_ptr, ftp_mptr->FTPDLEN) > 0)) {
					...
					if( ftp_mptr->FTPOPER != FTP_NEXT) {
						if( ftp_mptr->FTPOPER != FTP_CANCEL) {
							rcode = EDVSINVAL;
							ERROR_PRINT(rcode); 
						}
						break;
			...
			case FTP_PUT:
				...
				if(( ftpd_fd = mol_open(ftp_path_ptr,O_WRONLY)) == -1) {  
					rcode = (-errno);
					ERROR_PRINT(rcode); 
					break;
				}
