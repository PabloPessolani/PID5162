...
int main(int argc,  char *argv[])
{
	...
	if ((this_proc = dvk_getep(getpid())) < 0)
...
//	nw_test_icmp();
	...
	while (TRUE){
		...
		SVRDEBUG("dvk_receive\n");
		rcode = dvk_receive (ANY, &mq->mq_mess);
		if (rcode<0){
...
void inet_init(void)
{
	...
	do { 
		...
		SVRDEBUG("INET: dvk_wait4bind_T  rcode=%d\n", rcode);
		if (rcode == EDVSTIMEDOUT) {
			SVRDEBUG("INET: dvk_wait4bind_T TIMEOUT\n");
			continue ;
		}else if( rcode < 0) 
...
// 	Source MAC - It will be  overwritten by interface task
	...
	rcode = dvk_sendrec(ETH_PROC_NR, m_ptr);
	if(rcode != 0){
		ERROR_PRINT(rcode);
	}
