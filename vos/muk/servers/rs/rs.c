...
int rs_init(int dcid)
{
	...
	rs_ep = dvk_bind(dcid, RS_PROC_NR);
	
	if (rs_ep != RS_PROC_NR) {
		if(rs_ep == EDVSSLOTUSED){
			rrs_ptr = &rrs;
			rcode = dvk_getprocinfo(dcid, RS_PROC_NR, rrs_ptr);
			if(rcode < 0) ERROR_EXIT(rcode);
			...
			if( (TEST_BIT(rrs_ptr->p_rts_flags, BIT_REMOTE)) 
			 && (!TEST_BIT(rrs_ptr->p_misc_flags , MIS_BIT_RMTBACKUP))) {
				rcode = dvk_migr_start(dcid, RS_PROC_NR);
				if(rcode < 0) ERROR_EXIT(rcode);
				rcode = dvk_migr_commit(rs_lpid, dcid, RS_PROC_NR, local_nodeid);
				if(rcode < 0) ERROR_EXIT(rcode);
			}else{
...
#define SELFTASK (-1) 
		...
		switch(deamon_type){
			case LCL_BIND:
				rcode = dvk_lclbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep); 
				break;
			case REPLICA_BIND:
				rcode = dvk_replbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep); 
				break;
			case BKUP_BIND:
				rcode = dvk_bkupbind(dc_ptr->dc_dcid,SELFTASK,deamon_ep,deamon_nodeid);
				break;
			default:
				SVRDEBUG("NEVER HERE!!\n");
				rcode = EDVSINVAL;
...
//		rcode = sys_bindproc(deamon_ep, deamon_lpid, deamon_type);
		...
		if( deamon_type != BKUP_BIND ) {
			rcode = mol_bindproc(deamon_ep, mnxpid, SELFTASK, deamon_type);
			if( rcode < 0) 
				ERROR_RETURN(rcode);			
