...
//int rw_flag;      /* READING or WRITING */
{
   ...
   */
  if (who_e == pm_ep && (m_in.fd & _PM_SEG_FLAG))
  {
    seg = (int)m_in.m1_p2;
    usr = (int)m_in.m1_p3;
  ...
  if (!rw_flag) //READING
  {
    ...
    // {
      ...
      r = mnx_vcopy(fs_ep, localbuff, who_e, m_in.buffer, br);
  ...
  else
  {
    ...
    r = mnx_vcopy(who_e, m_in.buffer, fs_ep, localbuff, m_in.nbytes);
...
// int *completed;     /* number of bytes copied */
{
  ...
  *completed = 0;
  ...
  if (rw_flag == READING)
  {
    ...
    //mnx_vcopy(who_e, path, fs_ep, user_path, len);
    //r = mnx_vcopy(fs_ep, D, usr, seg, chunk);
    r = mnx_vcopy(fs_ep, (bp->b_data + off), usr, buff, chunk);
    SVRDEBUG("Resultado mnx_vcopy SERVIDOR -> CLIENTE %d\n", r);
    SVRDEBUG("chunk %d\n", chunk);
    //SVRDEBUG("buff %s\n", buff);
  ...
  else
  {
    ...
    //     (phys_bytes) chunk);
    //r = mnx_vcopy(usr, seg, fs_ep, D, chunk);
    r = mnx_vcopy(usr, buff, fs_ep, (bp->b_data + off), chunk);
    SVRDEBUG("Resultado mnx_vcopy CLIENTE -> SERVIDOR %d\n", r);
    SVRDEBUG("chunk %d\n", chunk);
    //SVRDEBUG("buff %s\n", buff);
    // SVRDEBUG("bp->b_data+off %s\n", bp->b_data+off);
