...
void pm_get_work(void)
{
	...
	while( TRUE) {
		...
		if ( (ret=dvk_receive(ANY, &pm_m_in)) != OK) {
			ERROR_PT_EXIT(ret);
		}
...
void pm_init(void)
{
	...
	if(pm_ep < 0) ERROR_PT_EXIT(pm_ep);
...
//	pm_kproc = malloc((dc_ptr->dc_nr_tasks + dc_ptr->dc_nr_procs)*sizeof(proc_usr_t));
	...
	if(pm_kproc == NULL) ERROR_PT_EXIT(rcode);
...
//	pm_kpriv = malloc((dc_ptr->dc_nr_tasks + dc_ptr->dc_nr_procs)*sizeof(priv_usr_t));
	...
	if(pm_kpriv == NULL) ERROR_PT_EXIT(rcode);
...
//	if(rcode < 0) ERROR_PT_EXIT(rcode);
	...
	if (clockTicks == -1)	ERROR_PT_EXIT(errno);
