...
int fs_getsysinfo()
{
  ...
  // if (OK != (s=sys_datacopy(SELF, src_addr, fs_who_e, dst_addr, len)))
  MUK_vcopy(s,SELF, src_addr, fs_who_e, dst_addr, len);
...
int fs_fcntl()
{
  ...
  switch (fs_m_in.request) {
		...
		case F_FREESP:	{
			...
			MUK_vcopy(r, fs_who_e, fs_m_in.name1, SELF, &flock_arg, sizeof(flock_arg))
