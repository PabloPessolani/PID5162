 ...
 * will be returned with an error code in fs_err_code.
 */
  ...
  // int contador =0;
	...
	MUKDEBUG("path=%s string=%s action=%d \n", path, string, action); 
  
	/* Is the path absolute or relative?  Initialize 'rip' accordingly. */
	if( *path == '/'){
		rip = fs_proc->fp_rootdir;
		MUKDEBUG("ABSOLUTE: fp_endpoint=%d i_num=%d\n", fs_proc->fp_endpoint, rip->i_num); 
	}else{
		rip = fs_proc->fp_workdir;
		MUKDEBUG("RELATIVE: fp_endpoint=%d i_num=%d\n", fs_proc->fp_endpoint, rip->i_num); 
	}
  
	// MUKDEBUG("fp_rootdir DEV= %d\n", (fs_proc->fp_rootdir)->i_dev);
	// MUKDEBUG("fp_workdir DEV= %d\n", (fs_proc->fp_workdir)->i_dev);
	// MUKDEBUG("rip->i_zone[0] ANTES =%d, DEV= %d \n", (mnx_dev_t) rip->i_zone[0], rip->i_dev);

	/* If dir has been removed or path is empty, return ENOENT. */
	if (rip->i_nlinks == 0 || *path == '\0') {
		fs_err_code = EDVSNOENT;
		MUKDEBUG("NIL_INODE por path nulo: i_nlinks=%d fs_err_code=%d\n", 
			rip->i_nlinks, fs_err_code); 
	...
	while (TRUE) {
		...
		if ( (new_name = get_name(path, string)) == (char*) 0) {
			...
			MUKDEBUG("bad path in user space\n"); 
			return(NIL_INODE);
		}
		if (*new_name == '\0' && (action & PATH_PENULTIMATE)) {
			if ( (rip->i_mode & I_TYPE) == I_DIRECTORY) {
				...
				put_inode(rip);
				fs_err_code = EDVSNOTDIR;     
				MUKDEBUG("last file of path prefix is not a directory: fs_err_code=%d\n", fs_err_code);  
				return(NIL_INODE);
			}
		}
		...
		if (rip == NIL_INODE) {
			MUKDEBUG("NIL_INODE = %s\n", NIL_INODE);
			if (*new_name == '\0' && (action & PATH_NONSYMBOLIC) != 0){
				return(dir_ip);
			} else {
				put_inode(dir_ip);
				MUKDEBUG("There is more path.  Keep parsing\n");                        
				return(NIL_INODE);
			}
		}
		...
		if (S_ISLNK(rip->i_mode)) {
            if (*new_name != '\0' || (action & PATH_OPAQUE) == 0) {
			   ...
			   put_inode(dir_ip);
			   if (++symloop > SYMLOOP) {
					   fs_err_code = EDVSLOOP;
					   put_inode(rip);
					   rip = NIL_INODE;
			   }
				if (rip == NIL_INODE) {
					MUKDEBUG("Fetch next component: fs_err_code=%d\n", fs_err_code);  
					return(NIL_INODE);
				}
			   continue;
            }
       ...
       if ((action & PATH_NONSYMBOLIC) != 0) {
               put_inode(rip);
               MUKDEBUG("Devolviendo dir_ip\n");                        
               return(dir_ip);
       } else {
               put_inode(dir_ip);
               MUKDEBUG("Devolviendo rip\n");                        
               return(rip);
       }
	}
}
...
//char *path;                    /* the path name to be parsed */
{
  ...
  * return NIL_INODE as function value and an error code in 'fs_err_code'.
  */
 ...
 * the reason for failure in 'fs_err_code'.
 */
 ...
 */
  ...
  if ((b = read_map(rip, (off_t) 0)) != NO_BLOCK) {
       ...
       if (sl > 0 && sl + tl <= MNX_PATH_MAX-1) {
               ...
               memmove(path, sp, sl);
               path[sl+tl] = 0;
               dup_inode(bip = path[0] == '/' ? fs_proc->fp_rootdir : ldip);
       }
  }
...
// MUKDEBUG("PASO string nulo\n");
  /* Check for NIL_INODE. */
  if (dirp == NIL_INODE) { return(NIL_INODE); }
// MUKDEBUG("PASO NIL_INODE\n");
  /* If 'string' is not present in the directory, signal error. */
  if ( (r = search_dir(dirp, string, &numb, LOOK_UP)) != OK) {
  	// MUKDEBUG("r=%d\n", r);
  	// MUKDEBUG("OK=%d\n", OK);
  	// MUKDEBUG("fs_err_code=%d\n", fs_err_code);
  	// MUKDEBUG("EDVSNOENT=%d\n", EDVSNOENT);
  	fs_err_code = r;
  return(NIL_INODE);
  }
// MUKDEBUG("PASO search_dir\n");
  /* Don't go beyond the current root directory, unless the string is dot2. */
  if (dirp == fs_proc->fp_rootdir && strcmp(string, "..") == 0 && string != dot2)
...
// MUKDEBUG("ANTES DE comparar por ROOT_INODE\n");
...
// MUKDEBUG("DIR_ENTRY_SIZE=%d\n", DIR_ENTRY_SIZE);  	

// MUKDEBUG("ldir_ptr->i_size=%d\n", ldir_ptr->i_size);
// MUKDEBUG("ldir_ptr->i_num=%d\n", ldir_ptr->i_num);    


  for (pos = 0; pos < ldir_ptr->i_size; pos += ldir_ptr->i_sp->s_block_size) {
  b = read_map(ldir_ptr, pos);  /* get block number */

// MUKDEBUG("Bloque b obtenido de read_map = %d\n", b);  	
...
// MUKDEBUG("ldir_ptr->i_sp->s_block_size%d\n", ldir_ptr->i_sp->s_block_size);
// MUKDEBUG("unsigned long %d\n", usizeof(unsigned long));
// MUKDEBUG("DIRSIZ * char = %d\n", DIRSIZ * usizeof(char));
// MUKDEBUG("usizeof (struct direct) = %d\n", usizeof (struct mnx_direct));
// MUKDEBUG("NR_DIR_ENTRIES(ldir_ptr->i_sp->s_block_size)%d\n", NR_DIR_ENTRIES(ldir_ptr->i_sp->s_block_size));

  /* Search a directory block. */
  for (dp = &bp->b_dir[0];
    dp < &bp->b_dir[NR_DIR_ENTRIES(ldir_ptr->i_sp->s_block_size)];
    dp++) {	
  	// MUKDEBUG("dp->d_name = %s\n", dp->d_name);
...
// MUKDEBUG("LOOK_UP=%d\n", LOOK_UP);
  ...
  if (flag != ENTER) {
    return(flag == IS_EMPTY ? OK : EDVSNOENT);
  }
