...
int fs_devctl()
{
  ...
  switch(fs_m_in.ctl_req) {
  case DEV_MAP:
      /* Check process number of new driver. */
      proc_nr_e= fs_m_in.driver_nr;
      if (isokendpt(proc_nr_e, &proc_nr_n) != OK)
      return(EDVSINVAL);

      /* Try to update device mapping. */
      result = map_driver(fs_m_in.dev_nr, proc_nr_e, fs_m_in.dev_style);
      if (result == OK)
      {
   ...
   */
      if(fs_proc_table[proc_nr_n].fp_execced) 
        {
          dev_up(fs_m_in.dev_nr);
        } 
      else 
        {
          fs_dmap_tab[fs_m_in.dev_nr].dmap_flags |= DMAP_BABY;
        }
      }
      break;
  case DEV_UNMAP:
      result = map_driver(fs_m_in.dev_nr, NONE, 0);
      break;
  default:
      result = EDVSINVAL;
  }
...
void dmap_unmap_by_endpt(int proc_nr_e)
{
  ...
  for (i=0; i<NR_DEVICES; i++)
    if(fs_dmap_tab[i].dmap_driver && fs_dmap_tab[i].dmap_driver == proc_nr_e)
...
int dmap_driver_match(int proc, int major)
{
  ...
  if(fs_dmap_tab[major].dmap_driver != NONE && fs_dmap_tab[major].dmap_driver == proc)
    return 1;
  return 0;
}
...
void dmap_endpt_up(int proc_e)
{
  int i;
  for (i=0; i<NR_DEVICES; i++) {
    if(fs_dmap_tab[i].dmap_driver != NONE
      && fs_dmap_tab[i].dmap_driver == proc_e
      && (fs_dmap_tab[i].dmap_flags & DMAP_BABY)) {
      fs_dmap_tab[i].dmap_flags &= ~DMAP_BABY;
      dev_up(i);
    }
  }
