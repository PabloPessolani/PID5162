 ...
 MUKDEBUG("fs_who_e=%d path=%X len=%d flag=%X\n", fs_who_e, path, len, flag);
  ...
  if (len > MNX_PATH_MAX) {
	...
	if (flag == M3 && len <= M3_STRING) {
		...
		//r = dvk_vcopy(fs_who_e, path, fs_ep, user_path, (len+1)); Ver si se hace del lado de OPEN.
		// r = dvk_vcopy(fs_who_e, path, SELF, user_path, (phys_bytes) len);
		MUK_vcopy( r, fs_who_e, path, SELF, user_path, (phys_bytes) len);
		MUKDEBUG("r =%d\n", r);
