...
/* Read a superblock. */
	...
	r = dev_io(DEV_READ, dev, fs_ep, sbbuf, SUPER_BLOCK_BYTES, _MIN_BLOCK_SIZE, 0);
	MUKDEBUG("output dev_io r=%d, \n", r);
	//Ver como hacer esta validacion
	if (r != _MIN_BLOCK_SIZE) {
		MUKDEBUG("_MIN_BLOCK_SIZE r=%d, \n", r);
		return EDVSINVAL;
	}

	// MUKDEBUG("memcpy de sb_ptr a sbbuf, de un tamaño de _MIN_BLOCK_SIZE \n");  
	...
	if (magic == SUPER_MAGIC || magic == conv2(BYTE_SWAP, SUPER_MAGIC)) {
		...
		// MUKDEBUG("SUPER_MAGIC \n"); 
		// MUKDEBUG("native %d\n", native);
		// MUKDEBUG("NO ENCONTRADO %d\n", i);
		return(EDVSINVAL);
	}
	// }
...
// MUKDEBUG(SUPER_BLOCK_FORMAT1, SUPER_BLOCK_FIELDS1(sp));
   ...
   */
    ...
    if ((sp->s_block_size % 512) != 0) {
		// MUKDEBUG("sp->s_block_size mod 512 != 0\n");
		return EDVSINVAL;
	}
	if (SUPER_SIZE > sp->s_block_size) {
		// MUKDEBUG("SUPER_SIZE > sp->s_block_size \n");    
		return EDVSINVAL;
	}
	if ((sp->s_block_size % V2_INODE_SIZE) != 0 ||
		(sp->s_block_size % V1_INODE_SIZE) != 0) {
		// MUKDEBUG("V2_INODE_SIZE %lu\n", V2_INODE_SIZE); 
		// MUKDEBUG("V1_INODE_SIZE %lu\n", V1_INODE_SIZE);     
		return EDVSINVAL;
	}
