...
//			ret = dvk_getdcinfo(dc_ptr->dc_dcid, dc_ptr);
    	...
    	case GET_PROCTAB: {
			...
			memcpy(lcl_ptr,  PROC_MAPPED(-dc_ptr->dc_nr_tasks), length);
    	...
    	case GET_PROC: {
			...
			ret = dvk_getprocinfo(dc_ptr->dc_dcid, p_nr, src_addr);
...
//			posix_memalign( (void**) &src_addr, getpagesize(), length);
//			if( src_addr == NULL) return (EDVSNOMEM);
			src_addr = proc_ptr = (proc_usr_t *) PROC_MAPPED(p_nr);
			ret = OK;			
#endif /* ALLOC_LOCAL_TABLE */
