...
int st_bindproc(message *m_ptr)	
{
  ...
  int rcode;
	...
	rcode = dvk_getprocinfo(dc_ptr->dc_dcid, sysproc_nr, sysproc_ptr);
	if( rcode < 0) ERROR_RETURN(rcode);
#else /* ALLOC_LOCAL_TABLE */			
	sysproc_ptr = (proc_usr_t *) PROC_MAPPED(sysproc_nr);
	rcode = OK;	
#endif /* ALLOC_LOCAL_TABLE */
	MUKDEBUG(PROC_USR_FORMAT,PROC_USR_FIELDS(sysproc_ptr));
	...
	if( (sysproc_nr+dc_ptr->dc_nr_tasks) >= dc_ptr->dc_nr_sysprocs) {
		...
		if(  TEST_BIT(sysproc_ptr->p_rts_flags, BIT_SLOT_FREE)) {
			...
			rcode = dvk_rmtbind(dc_ptr->dc_dcid,m_ptr->m3_ca1,
			...
			if(sysproc_ptr->p_endpoint == sysproc_ep){
				...
				if(sysproc_ptr->p_nodeid != sysproc_nodeid){
					...
					dvk_migr_start(dc_ptr->dc_dcid, sysproc_ptr->p_endpoint);
					dvk_migr_commit(PROC_NO_PID, dc_ptr->dc_dcid, 
						sysproc_ep, sysproc_nodeid);			
				}else{
				...
				MUKDEBUG("Another Remote process was using the slot: Unbind it before new bind %s\n",
						sysproc_ptr->p_name);						
				dvk_unbind(dc_ptr->dc_dcid,sysproc_ptr->p_endpoint);
				rcode = dvk_rmtbind(dc_ptr->dc_dcid,basename(m_ptr->m3_ca1),
							sysproc_ep,sysproc_nodeid);
				if( rcode < 0) ERROR_RETURN(rcode);
			}
		}
	...
	rcode = dvk_getpriv(dc_ptr->dc_dcid, sysproc_ep, sppriv_ptr);
	if( rcode < 0) ERROR_RETURN(rcode);
