...
//--------------------------------------------------------	
	...
	if( b <= 0) ERROR_PT_EXIT(EDVSINVAL); 	// tamaño de bloque no negativo o cero
	if( D <= 0) ERROR_PT_EXIT(EDVSINVAL); 	// tamaño de dispositivo no negativo o cero
	if( D < b) ERROR_PT_EXIT(EDVSINVAL); 	// tamaño de dispositivo mayor tamaño de bloque	
	if( (D % b) == 0){
		MUKDEBUG("D=%u múltiplo b=%u\n", D, b);
	} 
	else{
		ERROR_PT_EXIT(EDVSINVAL);// aseguro que el tamaño de disco sea multiplo 
	}
...
do{
	...
	switch(sl_msg.m_type){
		case DEV_CFULLR: {			
			...
			if(bytes < 0) ERROR_PT_EXIT(errno);
...
void init_m3ipc(void)
{
	...
	if(sc_ep < 0) ERROR_PT_EXIT(sc_ep);
	MUKDEBUG("sc_ep=%d\n", sc_ep);
	...
	if( endp_flag == 0) { // RDISK runs under MOL 
		...
		do {
			...
			MUKDEBUG("SLAVE dvk_wait4bind_T  rcode=%d\n", rcode);
			if (rcode == EDVSTIMEDOUT) {
				MUKDEBUG("CLIENT dvk_wait4bind_T TIMEOUT\n");
				continue ;
			} else if ( rcode < 0)
				ERROR_PT_EXIT(EXIT_FAILURE);
		} while	(rcode < OK);
		...
		if(mc_ep < 0) ERROR_PT_EXIT(mc_ep);
		MUKDEBUG("mc_ep=%d\n", mc_ep);
	}
   return(OK);
}
