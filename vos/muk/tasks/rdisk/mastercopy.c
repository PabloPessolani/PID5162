...
void init_m3ipcm()
{
	...
	if( endp_flag == 0) { // RDISK runs under MOL 
		...
		do {
			...
			MUKDEBUG("MASTER dvk_wait4bind_T  rcode=%d\n", rcode);
...
while (TRUE) { 
	...
	ret = dvk_receive( ANY , (long) &rd_mess);

	if( ret != OK){
		MUKDEBUG("dvk_receive ERROR %d\n",ret); 
	...
	if (r != EDVSDONTREPLY) { /*para MOL, /kernel/minix/molerrno.h no enviar respuesta (SIGN 201)*/
		...
		ret = dvk_send(dev_caller, &rd_mess); /*envío respuesta al cliente q solicitó*/
		if( ret != 0 ) {
