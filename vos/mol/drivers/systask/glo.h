...
EXTERN message m, *dvscmd_ptr;
EXTERN sigjmp_buf senv;
EXTERN int next_child;

EXTERN int nr_nodes;	/* current number of nodes that conform the DC (0 < nr_nodes <= dc_nr_nodes)*/
EXTERN int nr_init;		/* current number of nodes that conform the DC (0 <  nr_init <= nr_nodes <= dc_nr_nodes)*/
EXTERN int nr_radar;		/* current number of radar nodes */
...
EXTERN 	int 	nr_waitsts;		/* next member to send STS_WAIT_STATUS */

EXTERN char replica_name[MAXNODENAME]; /* MOLnn.dd  nn=nodeid vv=dcid*/
...
EXTERN	  unsigned long int			bm_nodes;		/* bitmap of active members */
...
EXTERN	  unsigned long int			bm_radar;		/* bitmap  of radar  members */
