...
void usage(char* errmsg, ...) {
	...
	// fprintf(stderr, "Usage: rdisk -vmid|-v [virtualmachine] -first|-f [minor number1] -First|-F [file image1] -second|-s [minor number2] -Second|-S [file image2] -buffer_size|-b [buffer size]\n");
...
//				TASKDEBUG("DRIVER - Offset (read) %X, Data: %s\n", devvec[m_device].localbuff, devvec[m_device].localbuff);			
			...
			TASKDEBUG("\n<DEV_SCATTER>\n");
			stbytes = 0;
			TASKDEBUG("\vm_ptr->vm_nr_nodes=%d, nr_nodes=%d\n",vm_ptr->vm_nr_nodes, nr_nodes);
			
			if( vm_ptr->vm_nr_nodes > 1) { /* PRIMARY;  MULTICAST to other nodes the device operation */
				if(primary_mbr == local_nodeid) {
...
/*open sobre el archivo imagen*/
	...
	TASKDEBUG("\vm_ptr->vm_nr_nodes=%d\n",vm_ptr->vm_nr_nodes);
...
if (bind_flag != 1){	
	...
	if (replicate_flag == 1){	
		...
		if ( primary_mbr != local_nodeid) {
			...
			rcode = mnx_migr_start(vm_ptr->vm_vmid, RDISK_PROC_NR);
			TASKDEBUG("mnx_migr_start rcode=%d\n",	rcode);
			rcode = mnx_migr_commit(rd_lpid, vm_ptr->vm_vmid, RDISK_PROC_NR, local_nodeid);
