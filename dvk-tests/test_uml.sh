...
echo "########################################################"
echo "# EJECUTAR EN UML !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!   #"
echo "########################################################"
read  -p "Enter para continuar... "
dmesg -c > /dev/null
#read  -p "Spread Enter para continuar... "
#mkdir /var/run/spread
#/usr/local/sbin/spread -c /etc/spread.conf > /tmp/dvs/spread.txt &
...
./test_dc_init -d $dcid
dmesg -c >> /tmp/dvs/dmesg.txt
read  -p "TCP LZ5 BAT PROXY Enter para continuar... "
...
#./tcp_proxy node$rmt $rmt > /tmp/dvs/node$rmt.txt 2> /tmp/dvs/error$rmt.txt &
...
#read  -p "Enter para continuar... "
#/usr/src/dvs/dvk-proxies/tipc_proxy_bat -bBZ -n node$rmt -i $rmt > /tmp/dvs/node$rmt.txt 2> /tmp/dvs/error$rmt.txt &	
sleep 5
netstat  -nat
#tipc bearer list
#tipc nametable show
