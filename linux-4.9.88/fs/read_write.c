 ...
 *
 * @type: One of %CHECK_IOVEC_ONLY, %READ, or %WRITE.
 * @kvector: Pointer to the kernel space array.
 * @nr_segs: Number of elements in userspace array.
 *
 * Return: The total number of bytes covered by the iovec array on success
 *   or a negative error code on error.
			      ...
			      unsigned long nr_segs)
{
	...
	DVKDEBUG(DBGPARAMS, "type=%d nr_segs=%d\n", type, nr_segs);

	if (nr_segs == 0) return(0);

	if (nr_segs > UIO_MAXIOV) ERROR_RETURN(-EINVAL);
	
	ret = 0;
	for (seg = 0; seg < nr_segs; seg++) {
		buf = kvector[seg].iov_base;
		len = (ssize_t)kvector[seg].iov_len;

		if (len < 0) ERROR_RETURN(-EINVAL);
