...
/*--------------------------------------------------------------*/
 long send_ack_rmt2lcl(struct proc *rmt_ptr, struct proc *lcl_ptr, int rcode)
{
	dc_desc_t *dc_ptr;
...
DVKDEBUG(DBGPARAMS,"dcid=%d src_ep=%d dst_ep=%d rcode=%d\n",dc_ptr->dc_usr.dc_dcid, rmt_ptr->p_usr.p_endpoint, lcl_ptr->p_usr.p_endpoint, rcode);
	...
	if(!test_bit(BIT_SENDING, &lcl_ptr->p_usr.p_rts_flags))  	ERROR_RETURN(EDVSACKDST);
...
DVKDEBUG(DBGPARAMS,"dcid=%d src_ep=%d dst_ep=%d ack=%d rcode=%d\n",
	...
	do {
		ret = OK;
		if( IT_IS_LOCAL(rmt_ptr)) 			{ret= EDVSLCLPROC; break;}
