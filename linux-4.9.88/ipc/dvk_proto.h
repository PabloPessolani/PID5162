...
long do_dc_end(dc_desc_t *dc_ptr);
 long do_unbind(dc_desc_t *dc_ptr, struct proc *proc_ptr);
int kernel_warn2proc( dc_desc_t *dc_ptr, struct proc *caller_ptr, struct proc *warn_ptr);
 long send_rmt2lcl(proxy_hdr_t *usr_h_ptr, proxy_hdr_t *h_ptr, struct proc *src_ptr, struct proc *dst_ptr);
 long notify_rmt2lcl(proxy_hdr_t *usr_h_ptr, proxy_hdr_t *h_ptr, struct proc *src_ptr,struct proc *dst_ptr, int update_proc);
 long send_ack_rmt2lcl(struct proc *src_ptr,struct proc *dst_ptr, int rcode);
 long copyout_data_rmt2lcl(struct proc *caller_ptr, struct proc *src_ptr,struct proc *dst_ptr, 
		proxy_hdr_t *h_ptr, proxy_payload_t *usr_pay_ptr);
 long copyin_data_rmt2lcl(struct proc *caller_ptr, struct proc *src_ptr, struct proc *dst_ptr, 
		proxy_hdr_t *h_ptr, proxy_payload_t *usr_pay_ptr);
 long copyout_rqst_rmt2lcl( struct proc *caller_ptr, struct proc *src_ptr, struct proc *dst_ptr, 
		proxy_hdr_t *h_ptr, proxy_payload_t *usr_pay_ptr);
 long copylcl_rqst_lcl2lcl(struct proc *rproxy_ptr, struct proc *rmt_ptr, 
 ...
 int mm_proxy_conn(int px_nr, int status);
 long mm_put2lcl(proxy_hdr_t *usr_hdr_ptr, proxy_payload_t *usr_pay_ptr);
 long mm_get2rmt(proxy_hdr_t *usr_hdr_ptr, proxy_payload_t *usr_pay_ptr, long timeout_ms);
 long mm_node_up(char *node_name, int nodeid, int px_nr);
 long mm_exit_unbind(long code);
 long mm_wakeup(int dcid, int proc_ep);
...
/* protos of routines to access non exported symbols */
 long mm_sys_wait4(pid_t, int __user *,int , struct rusage __user *);
void put_task_struct(struct task_struct *);
//long sched_setaffinity(pid_t , const struct cpumask *);
