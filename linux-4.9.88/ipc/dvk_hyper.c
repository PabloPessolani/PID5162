...
/* 1- the caller: calling ipc_unbind			*/
...
DVKDEBUG(INTERNAL,"sched_setaffinity param_pid=%d\n",param_pid);
sched_setaffinity(param_pid, &pap_mask);
...
//			sys_wait4(other_pid, (int __user *)&ret, 0, NULL);
//			schedule();				
// ANALIZAR si el proceso que se esta matando es HIJO del MATADOR de tal modo de hacer el  sys_wait4 necesario para no quedar ZOMBIE.
...
//	UNLOCK_ALL_PROCS(dc_ptr, tmp_ptr, i);
	...
	FOR_EACH_PROC(dc_ptr, i) {
		...
		while( !test_bit(BIT_SLOT_FREE, &proc_ptr->p_usr.p_rts_flags)) {
			if( IT_IS_LOCAL(proc_ptr) || test_bit(MIS_BIT_RMTBACKUP, &proc_ptr->p_usr.p_misc_flags) ) {
				task_ptr = proc_ptr->p_task;
				sys_wait4(task_pid_nr(task_ptr), (int __user *)&ret, 0, NULL);
				RUNLOCK_PROC(proc_ptr);	
				WUNLOCK_DC(dc_ptr);
				schedule();
				WLOCK_DC(dc_ptr);
...
/*----------------------------------------------------------------*/
 int k_proxies_bind(char *px_name, int px_nr, int spid, int rpid, int maxbytes)
{
	struct task_struct *stask_ptr, *rtask_ptr;	
	struct proc *sproxy_ptr, *rproxy_ptr;
	proc_usr_t  *sp_ptr, *rp_ptr;
...
/* CONNECT_PROXIES or DISCONNECT_PROXIES 			*/
/*----------------------------------------------------------------*/
 int k_proxy_conn(int px_nr, int status)
{
	struct proc *sproxy_ptr, *rproxy_ptr;
	proc_usr_t *s_ptr, *r_ptr;
	proxies_t *px_ptr;
