...
#define DVK_IPC(x1,x2,x3,x4,x5,x6) ipc(x1,x2,x3,x4,x5,x6)
...
long dvk_vcopy(int src_ep, void *src_addr, int dst_ep, void *dst_addr, int bytes)
{
	...
	ret = DVK_IPC(ipc_op, src_ep, src_addr,dst_ep,dst_addr, bytes);
...
long dvk_dvs_end(void)
{
	...
	ret = DVK_IPC(	ipc_op, 0 , 0L, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC
...
long dvk_dc_init(dc_usr_t *dcu_ptr)
{
	...
	ret = DVK_IPC(	ipc_op, 0 , 0L, 0L, dcu_ptr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC
...
long dvk_dc_end(int dcid)
{
	...
	ret = DVK_IPC(	ipc_op, dcid , 0L, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_getdvsinfo(dvs_usr_t *dvsu_ptr)
{
	...
	ret = DVK_IPC(	ipc_op, 0 , 0L, 0L, dvsu_ptr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_proxies_unbind(int pxid)
{
	...
	ret = DVK_IPC(	ipc_op, pxid , 0L, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_node_down(int nodeid)
{
	...
	ret = DVK_IPC(	ipc_op, nodeid , 0L, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_rcvrqst_T(message *mptr, long timeout)
{
	...
	ret = DVK_IPC(	ipc_op, 0 , 0L, 0L, mptr, timeout);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_getdcinfo(int dcid, dc_usr_t *dcu_ptr)
{
	...
	ret = DVK_IPC(	ipc_op, dcid , 0L, 0L, dcu_ptr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_getnodeinfo(int nodeid, node_usr_t *node_ptr)
{
	...
	ret = DVK_IPC(	ipc_op, nodeid , 0L, 0L, node_ptr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_relay(int endpoint, message *mptr)
{
	...
	ret = DVK_IPC(	ipc_op, endpoint , 0L, 0L, mptr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_wakeup(int dcid, int dst_ep)
{
	...
	ret = DVK_IPC(	ipc_op, dcid , dst_ep, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_put2lcl(cmd_t *header, proxy_payload_t *payload)
{
	...
	ret = DVK_IPC(	ipc_op, 0 , header, payload, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC
...
long dvk_add_node(int dcid, int nodeid)
{
	...
	ret = DVK_IPC(	ipc_op, dcid , nodeid, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_dvs_init(int nodeid, dvs_usr_t *dvsu_ptr)
{
	...
	ret = DVK_IPC(ipc_op, nodeid , 0L, 0L, dvsu_ptr, 0L);
...
long dvk_proxy_conn(int pxid, int status)
{
	...
	ret = DVK_IPC(	ipc_op, pxid , status, 0L, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_wait4bindep_X(int cmd, int endpoint, unsigned long timeout)
{
	...
	ret = DVK_IPC(	ipc_op, cmd , endpoint, timeout, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC
...
long dvk_unbind_T(int dcid, int endpoint, unsigned long timeout)
{
	...
	ret = DVK_IPC(	ipc_op, dcid , endpoint, timeout, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_send_T(int endpoint , message *mptr, long timeout)
{
	...
	ret = DVK_IPC(	ipc_op, endpoint, 0L, 0L, mptr, timeout);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_notify_X(int nr , int endpoint, int value)
{
	...
	ret = DVK_IPC(	ipc_op, nr, endpoint, value, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_setpriv(int dcid , int endpoint, priv_usr_t *priv)
{
	...
	ret = DVK_IPC(	ipc_op, dcid, endpoint, 0L, priv, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_get2rmt_T(cmd_t *header, proxy_payload_t *payload , long timeout)
{
	...
	ret = DVK_IPC(	ipc_op, 0, header, payload, NULL, timeout);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_node_up(char *name, int nodeid,  int pxid)
{
	...
	ret = DVK_IPC(	ipc_op, nodeid, pxid, 0L, name, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_getproxyinfo(int pxid, proc_usr_t *sproc_usr, proc_usr_t *rproc_usr)
{
	...
	ret = DVK_IPC(	ipc_op, pxid, sproc_usr, rproc_usr, NULL, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC	
...
long dvk_getprocinfo(int dcid, int p_nr, proc_usr_t *p_usr)
{
	...
	ret = DVK_IPC(	ipc_op, dcid, p_nr, 0L, p_usr, 0L);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		
...
long dvk_proxies_bind(char *name, int pxid, int spid, int rpid, int maxcopybuf)
{
	...
	ret = DVK_IPC(	ipc_op, pxid, spid, rpid, name, maxcopybuf);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		 
...
long dvk_migrate_X(int cmd, int pid, int dcid, int endpoint, int nodeid)
{
	...
	ret = DVK_IPC(	ipc_op, cmd, pid, dcid, endpoint, nodeid);
	LIBDEBUG(DBGPARAMS,"ipc ret=%d\n",ret);	
	if (ret < 0) {ret = (-errno); ERROR_PRINT(ret);}
	errno = 0;
#else // CONFIG_DVKIPC		 
