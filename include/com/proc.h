...
struct tramp_s {
	...
	struct proc	*t_dst;
	struct proc	*t_rqtr;
	void 	*t_saddr;	/* virtual address copy from 	*/
  	void 	*t_daddr;	/* virtual address copy to 	*/
  	int	t_bytes;	
};
typedef struct tramp_s tramp_t;
...
struct proc {
  ...
  tramp_t	p_tramp;		/* used by USR2USR, KRN2USR , USR2KRN copy functions */
...
// struct timer_list p_timer; 	/* IPC timeout TIMER	*/
  ...
  struct pid_namespace *p_pid_ns;    /* para apuntar al ns de PIDs */
...
#define PCOPY_FIELDS(p) p->p_tramp.t_src->p_usr.p_endpoint, \
					p->p_tramp.t_dst->p_usr.p_endpoint, p->p_tramp.t_rqtr->p_usr.p_endpoint,\
					p->p_tramp.t_saddr, p->p_tramp.t_daddr, p->p_tramp.t_bytes
...
struct dc_struct {
	...
	struct pid_namespace *dc_pid_ns;    /* para apuntar al ns de PIDs */
	struct nsproxy dc_nsproxy;			/* a set of pointer to different namespaces */
