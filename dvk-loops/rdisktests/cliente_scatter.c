...
void  main ( int argc, char *argv[] )
{
    ...
    if ( argc != 6)
    {
        printf( "Usage: %s <vmid> <clt_nr> <svr_nr> <position> <bytesCount> \n", argv[0] );
        exit(1);
    }
	...
	iovec[0].iov_size=20; /*sólo 20 bytes del buffer voy a copiar*/
	...
	iovec[1].iov_size=20; /*sólo 20 bytes del buffer voy a copiar*/
	...
	iovec[2].iov_size=20; /*sólo 20 bytes del buffer voy a copiar*/
