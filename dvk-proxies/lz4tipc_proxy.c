...
int decompress_payload( proxy_desc_t *pxd_ptr)
{
	...
	PXYDEBUG("RPROXY: INPUT px_maxRsize=%d px_maxCsize=%d \n", 
	...
	total = sizeof(proxy_hdr_t);
   	...
   	while ((n = recv(rconn_sd, p_ptr, (total-received), 0 )) > 0) {
        ...
        if (received >= sizeof(proxy_hdr_t)) {  
			PXYDEBUG("RPROXY: " CMD_FORMAT,CMD_FIELDS(px_desc.px_header));
        	return(OK);
        } else {
...
//	slot_t slot, *s_ptr;
    ...
    if(payload_size != 0) {
		...
		switch(px_desc.px_header->c_cmd){
			...
			case CMD_COPYOUT_DATA:
				PXYDEBUG("RPROXY: "VCOPY_FORMAT, VCOPY_FIELDS(px_desc.px_header)); 
				if( px_desc.px_header->c_len > 0){
					...
					PXYDEBUG("RPROXY: " CMD_XFORMAT, CMD_XFIELDS(px_desc.px_header));
					if(px_desc.px_header->c_flags == CMD_LZ4_FLAG){
						...
						rcode = dvk_put2lcl(px_desc.px_header, px_desc.px_payload);
					}else{ /* the payload has not been decompressed */
						rcode = dvk_put2lcl(px_desc.px_header, px_desc.px_comp_pl);		
					}
	...
	retry = BIND_RETRIES;
	do {
		ret = dvk_bind(px_desc.px_header->c_dcid, PROXY);
		if(ret < PROXY) {
			ERROR_PRINT(ret);
	...
	if( s_ptr->s_owner !=  px_desc.px_header->c_snode){
		...
		dvk_unbind( px_desc.px_header->c_dcid, PROXY);	
		ERROR_RETURN(rcode); /* !!!! return the original return code !!!! */		
	}
	...
	if( rcode ){
		ret = dvk_put2lcl(px_desc.px_header, px_desc.px_payload);
		if( ret) {
			ERROR_PRINT(ret);
			ERROR_RETURN(rcode);
		}
...
void pr_init(void) 
{
	...
	PXYDEBUG("px_desc.px_header=%p px_desc.px_payload=%p diff=%d\n", 
...
int  ps_send_payload(char *pl_ptr) 
{
	...
	if( px_desc.px_header->c_len <= 0) ERROR_EXIT(EDVSINVAL);
...
int  ps_start_serving(void)
{
    ...
    while(1) {
		...
		ret = dvk_get2rmt(px_desc.px_header, px_desc.px_payload);       
		...
		PXYDEBUG("SPROXY: %d "HDR_FORMAT,pid, HDR_FIELDS(px_desc.px_header)); 
		sm_flag = TRUE;
		switch(px_desc.px_header->c_cmd){
			case CMD_SEND_MSG: /* reply for REMOTE CLIENT BINDING*/	
				...
				if( ret == 0 ){
					...
					PXYDEBUG("SPROXY: CMD_SEND_MSG dont match REMOTE CLIENT BINDING ret=%d\n",ret);					
				}
				break;	
			case CMD_SNDREC_MSG:
			case CMD_REPLY_MSG:
			...
			case CMD_COPYIN_DATA:
			case CMD_COPYOUT_DATA:
				PXYDEBUG("SPROXY: "VCOPY_FORMAT, VCOPY_FIELDS(px_desc.px_header)); 
				break;
			default:
				break;
		}
